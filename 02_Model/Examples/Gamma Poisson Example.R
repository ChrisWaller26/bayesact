...
sev_par2_vec = exp(c(EMEA = 3, USC = 4))
...
model_post_samples =
  ...
  transmute(
    s1_emea = exp(b_loss_s1_Intercept), 
    s1_usc  = exp(b_loss_s1_Intercept +
                    b_loss_s1_regionUSC),
    
    shape_emea = exp(b_shape_loss_Intercept), 
    shape_usc  = exp(b_shape_loss_Intercept 
                     + b_shape_loss_regionUSC
                     ),
...
model_output =
  ...
  bind_rows(
    data.frame(
      ...
      shape_emea = sev_par2_vec["EMEA"], 
      shape_usc  = sev_par2_vec["USC"],
