...
freq_data =
  ...
  mutate(
    ...
    freq_shape = freq_shape[region],
    claimcount_fgu = 
      rnbinom(freq_n, mu = freq_mu, size = freq_shape)
  )
...
model_post_samples =
  ...
  transmute(
    ...
    shape_emea = 
      exp(b_shape_claimcount_Intercept), 
    shape_usc  = 
      exp(b_shape_claimcount_Intercept +
            b_shape_claimcount_regionUSC)
  )
...
model_output =
  ...
  bind_rows(
    data.frame(
      ...
      shape_emea = freq_shape["EMEA"], 
      shape_usc  = freq_shape["USC"]
    )
  )
